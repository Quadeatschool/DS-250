...
LetsPlot.setup_html(isolated_frame=True)  # Ensure proper HTML rendering
...
df_sorted = df.groupby('airport').agg({'Weather_Delay_Proportion': 'mean'}).reset_index()
df_sorted = df_sorted.sort_values(by='Weather_Delay_Proportion', ascending=False)

# Generate the barplot with Lets-Plot
ggplot(df_sorted) + \
    geom_bar(aes(x='airport', y='Weather_Delay_Proportion'), stat='identity', fill='blue') + \
    ggtitle("Weather Delay Proportion by Airport") + \
    theme(axis_text_x=element_text(angle=45))  # Rotate x-axis labels for readability
