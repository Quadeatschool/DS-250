...
FROM
    collegeplaying AS c
JOIN
    Schools AS sc ON c.schoolID = sc.schoolID
JOIN
    Salaries AS s ON c.playerID = s.playerID
WHERE
    sc.schoolID LIKE '%idbyuid%'
ORDER BY
    s.salary DESC;
"""

dfone = pd.read_sql_query(query, con)
dfone
...
ORDER BY batting_average DESC, playerID
LIMIT 5;"""
df_bat_av = pd.read_sql_query(queryB, con)
df_bat_av
...
queryC="""SELECT 
    playerID, 
    ROUND(CAST(SUM(H) AS FLOAT)/SUM(AB), 3) AS career_batting_average
FROM Batting
GROUP BY playerID
HAVING SUM(AB) >= 100
ORDER BY career_batting_average DESC, playerID
LIMIT 5;"""

df_bat_av = pd.read_sql_query(queryC, con)
df_bat_av
...
SL3 has very little data to track in this graph I don't know alot about baseball, but I assume they were a short lived team.
...
df_teams = pd.read_sql_query(query, con)
...
plot = (ggplot(df_teams, aes(x='yearID', y='Wins', color='teamID')) +
        geom_line(size=1.5) +
        geom_point() +
        labs(title='Wins Over Time: SL3 vs BSN',
             x='Year',
             y='Wins',
             color='Team') +
        theme_minimal())
...
q = '''
SELECT playerID, ROUND(CAST(H AS FLOAT)/AB, 3) AS batting_average, AB
FROM batting
LIMIT 2;
'''
